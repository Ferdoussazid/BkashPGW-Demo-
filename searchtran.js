...
const searchUrl = 'https://tokenized.sandbox.bka.sh/v1.2.0-beta/tokenized/checkout/general/search/searchTransaction';
...
const searchTransaction = async (trxID) => {
    try {
        ...
        console.error('Failed to search details:', error.response ? error.response.data : error.message);
        return { success: false, message: 'Failed to get search details', error: error.response ? error.response.data : error.message };
    }
};
...
app.get('/callback', async (req, res) => {
    ...
    if (status === 'success') {
        ...
        if (response.trxID) {
            let searchResponse = await searchTransaction(response.trxID);
